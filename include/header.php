    <div id="header-main">

            <div class="picture">
						<a href="centrum-medyczne-riemer">
							<img src="obrazy/logo.png" />
						</a>
                    </div>
                    <div class="header-social">
                        <a class="yt-top" href="https://www.youtube.com/channel/UCDUJUOviojf4vw22q7m-K8w" target="_blank"></a>
                        <a class="fb-top" href="https://www.facebook.com/centrumriemer/" target="_blank"></a>
                    </div>

    </div>
    <div id="header-menu">
					<ol>
				        <li>
                            <a href="centrum-medyczne-riemer">
                                  Start
                              </a>
                        </li>
				        <li>
                            <a href="co-nowego-slychac">
									Nowości
                            </a>
                        </li>
				        <li>
                              <a style="cursor:pointer;" onClick=”return true”>Oferta</a>
								<ul>
                                    <li>
									   <a href="transport-pacjenta-samolotem">
												Transport Air Ambulance
                                        </a>
                                    </li>
                                    <div style="clear:both;"></div>
                                    <li>
									   <a href="medyczna-asysta-lotnicza">
												Medyczna asysta lotnicza
                                        </a>
                                    </li>
                                    <div style="clear:both;"></div>
                                    <li>
                                        <a href="transport-pacjenta-pojazdem">
												Transport Ambulansem
                                        </a>
                                    </li>
                                    <div style="clear:both;"></div>
                                    <li>
                                        <a href="szkolenia-kursy-pierwsza-pomoc">
												Szkolenia i kursy
                                        </a>
                                    </li>
                                    <div style="clear:both;"></div>
                                    <li>
								        <a href="obstawy-imprez-masowych">
												Obstawy imprez masowych
								        </a>
                                    </li>
								</ul>
						  </li>
                            <li>
                                <a href="centrum-medyczne-galeria">
									Galeria
                                </a>
                            </li>
                            <li>
                              <a style="cursor:pointer;" onClick=”return true”>Referencje</a>
								<ul>
                                    <li>
									   <a href="alter-art-referencje">
												Alter-Art
                                        </a>
                                    </li>
                                    <div style="clear:both;"></div>
                                    <li>
									   <a href="europ-assistance-referencje">
												Europ Assistance
                                        </a>
                                    </li>
                                    <div style="clear:both;"></div>
                                    <li>
                                        <a href="legia-warszawa-referencje">
												Legia Warszawa
                                        </a>
                                    </li>
								</ul>
						  </li>
                            <li>
                                <a href="czym-jest-centrum-riemer">
									O nas
                                </a>
                            </li>
                            <li>
                                <a href="skontaktuj-sie-z-nami">
									Kontakt
                                </a>
                            </li>
					</ol>
		</div>
