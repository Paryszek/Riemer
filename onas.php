<!DOCTYPE HTML>
<html lang="pl">
<head>
<title></title>
<?php require_once 'include/meta.php'; ?>
</head>

<body>
	<div id="container">
		<?php require_once 'include/header.php'; ?>
		<div id="content">
			<div class="title">
				<i class="icon-users"></i> O nas
			</div>
          <img src="obrazy/onas.jpg" style="float:left; box-shadow: 1px 1px 2px #000000; border:3px solid #ffffff; width:300px; height:auto; margin:5px; margin-right:10px;" /> <div style="text-align:justify; font-size:18px;">Centrum Medyczne Riemer działa na rynku usług medycznych od 16 lat. Jesteśmy również operatorem systemu Państwowe Ratownictwo Medyczne w województwie Łódzkim.
Chcielibyśmy przedstawić ofertę kompleksowego zabezpieczania imprez masowych pod względem medyczno-ratowniczym.
Dysponujemy nowoczesnymi ambulansami, wyposażonymi zgodnie z wymogami
Ustawy o Państwowym Ratownictwie Medycznym oraz europejską normą w PN-EN 1867.
Naszym atutem jest kadra medyczna, którą stanowi wyselekcjonowany personel, pozyskany w wyniku restrykcyjnej rekrutacji, weryfikującej wiedzę teoretyczną i umiejętności praktyczne kandydatów. Jako firma medyczna kładziemy ogromny nacisk na szkolenie kadry czego efektem są jej ogromne osiągnięcia, zarówno w kraju 
jak i arenie międzynarowej. 
Nadrzędnym, przyjętym przez nas celem działalności jest wysoka jakość świadczonych usług.<br /><br/>
<div style="text-align:center;">
 <a class="btn" href="skontaktuj-sie-z-nami">Skontaktuj się z nami</a>
 <a class="btn" href="transport-pacjenta-samolotem">Sprawdź naszą ofertę</a>
 <a class="btn" href="sprzet-karetki-samoloty">Zobacz nasz sprzęt</a>
</div>
</div>

		</div>
		<?php require_once 'include/footer.php'; ?>
</div>
<script type="text/javascript" src="script/jquery-1.11.3.min.js"></script>
<script type="text/javascript" src="script/main.js"></script>
</body>
</html>
